...
test('functional, Docker, validation, and security steps never become transient', () => {
  for (const step of [
...
test('injected-client orchestration requests exactly one canonical transient rerun', async () => {
  ...
  const transientJob = job({ id: 10, step: recoveryConfig.transientSteps[2] });
