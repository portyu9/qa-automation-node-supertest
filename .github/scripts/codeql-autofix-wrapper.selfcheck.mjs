...
test('caller targets only the requested alerts with bounded scope', () => {
  assert.deepEqual(config.targetAlertNumbers, [3, 4]);
...
test('autofix cannot directly change workflow, dependency, or policy files', () => {
  for (const denied of ['.github/workflows/', '.github/dependabot.yml', '.github/dependency-governance.json', '.github/dependency-recovery.json', '.github/codeql-autofix.json', 'package.json', 'package-lock.json', 'Dockerfile']) {
    assert.ok(config.deniedPaths.includes(denied), `${denied} must stay denied`);
  }
});

test('wrapper pins canonical controller and every external action immutably', () => {
...
test('autofix control plane requires manual review', () => {
  for (const protectedPath of ['.github/codeql-autofix.json', '.github/workflows/codeql-autofix.yml', '.github/scripts/codeql-autofix-wrapper.selfcheck.mjs']) {
